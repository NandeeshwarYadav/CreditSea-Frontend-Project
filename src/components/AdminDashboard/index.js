...
const AdminDashboard= ()=>{
   ...
   // const [actionStatus,setActionStatus]=useState(false)
    ...
    const getData=async ()=>{
        const url=`https://creditsea-backend-project.onrender.com/admin/dashboard`
        const options={
            method:"GET"
        }
    ...
    const onClickActionBtn =async (id,status) => {
  const url = `https://creditsea-backend-project.onrender.com/admin/verify-loan/${id}`
  const options = {
    method: 'PUT',
    headers: {
  ...
  //  console.log(usersList)
    ...
    const CardDisplay=()=> {
        ...
        const totalAmountDisbursed = usersList?.totalAmountDisbursed
         ...
         const totalUsers = usersList?.totalUsers
      // console.log(totalSavings)
    return <CardContainer>
        <Card>
        <CardLeft>
        <RiTeamFill  style={{color:"white",fontSize:"40px"}}   />
       </CardLeft>
       <CardRight>
        <CardTitle>{totalUsers} <br/> Users</CardTitle>
       </CardRight>
       </Card>
       <Card>
        <CardLeft>
    ...
    recentApplications.map(each=>(<div key={each.id}>
        <CardHeadingsContainer>
            ...
            <CgProfile   style={{fontSize:"30px"}} />
                ...
                <UserCardActionBtn type="button"  onClick={()=>onClickActionBtn(each.id,each.status )}  status={each.status}  >{each.status}</UserCardActionBtn>
        </CardHeadingsContainer>
